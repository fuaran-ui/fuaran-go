...
func TestVisibleFalseRemovesTheNodeEntirely(t *testing.T) {
	html := renderFixture(t, "node-visible", BindingSources{"banner.shown": wire.Bool(false)})
...
func TestVisibleTrueRendersTheNode(t *testing.T) {
	html := renderFixture(t, "node-visible", BindingSources{"banner.shown": wire.Bool(true)})
...
func TestVisibleUnresolvedRendersTheNode(t *testing.T) {
	...
	html := renderFixture(t, "node-visible", nil)
...
func TestVisibleComputedPredicateResolvesThroughTheScalarPath(t *testing.T) {
	...
	assertContains(t, renderFixture(t, "node-visible", BindingSources{"cart.itemCount": wire.Int(9)}),
		shown, "9 > 3")
	assertAbsent(t, renderFixture(t, "node-visible", BindingSources{"cart.itemCount": wire.Int(1)}),
		shown, "1 is not > 3")
}
...
func TestVisibleDefaultVisibleSpellingNeedsNoHostState(t *testing.T) {
	...
	assertAbsent(t, renderFixture(t, "node-visible", BindingSources{"banner.dismissed": wire.Bool(false)}),
...
func TestSwitchPredicateCaseTakenOnResolvedTrue(t *testing.T) {
	html := renderFixture(t, "switch-predicate", BindingSources{"cart.empty": wire.Bool(true)})
...
func TestSwitchFirstMatchWinsRunsOverTheAuthoredOrder(t *testing.T) {
	...
	html := renderFixture(t, "switch-predicate", BindingSources{
...
func TestSwitchWithNoSelectorSelectsByPredicateAlone(t *testing.T) {
	assertContains(t, renderFixture(t, "switch-predicate-only", BindingSources{"form.valid": wire.Bool(true)}),
		"Ready to send", "a when-only switch needs no selector")
	assertContains(t, renderFixture(t, "switch-predicate-only", BindingSources{"form.valid": wire.Bool(false)}),
...
func TestSwitchComputedSelectorSelectsTheMatchingCase(t *testing.T) {
	...
	html := renderFixture(t, "switch-on-transform-scalar", nil)
